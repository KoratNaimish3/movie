...
//api to get now playing from TMDB API
...
export const addshow = async (req, res) => {
    try {
        ...
        if (!movie) {
            ...
            if (movieApiData && movieCreditsdata) {
                const movieDetails = {
                    _id: movieId,
...
//API to get all shows fron the database
export const getShows = async (req, res) => {
    try {
        ...
        console.log("Error in getShows :-", error)
        res.status(500).json({ success: false, message: "Internal Server Error" })
    }
}
...
export const getSingleShow = async (req, res) => {
    try {
        ...
        //get all upcoming shows the movie
        ...
        console.log("Error in getSingleShows :-", error)
        res.status(500).json({ success: false, message: "Internal Server Error" })
    }
}
